...
def lookForElements(layout, aprxFileName):
    with writeToOutputFile() as outputFile:
        ...
        for element in layout.listElements(wildcard='*'):
            elementFragments = []
            elementFragments.append(removeAPRXVersionNumber(aprxFileName))
...
def writeToOutputFile():
    ...
    file = os.path.join(os.getcwd(), '20YYiso3nn', 'GIS','3_Mapping', '32_Map_Templates', "325_Misc", "3253_element_locations","template_positions_" + today + ".txt")
    return open(file, 'a+')
